class Public::PostsController < ApplicationController
  ...
  def index
    ...
    if params[:latest]
      @posts = all_posts.latest.page(params[:page]).joins(:user).where(users: { is_active: true })
    elsif params[:old]
      @posts = all_posts.old.page(params[:page]).joins(:user).where(users: { is_active: true })
    elsif params[:liked]
      @posts = all_posts.liked.page(params[:page]).joins(:user).where(users: { is_active: true })
    else
